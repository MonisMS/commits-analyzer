...
export async function syncUserData(
  ...
  if (rateLimit && rateLimit.remaining < 200) {
    throw new Error(
      `Rate limit too low: ${
        rateLimit.remaining
      } remaining. Resets at ${rateLimit.reset.toLocaleTimeString()}`
    );
  }
  console.log(
  ...
  console.log(
    `Sync completed: ${totalCommits} commits from ${totalRepos} repositories`
  );
  console.log(
    `Rate limit after sync: ${finalRateLimit?.remaining}/${finalRateLimit?.limit} remaining`
  );
